...
import { CheckSquare, Plus, Calendar, Settings, TrendingUp, Clock, Target, Users, Home } from 'lucide-react'
...
export default function DashboardActions() {
  ...
  const stats = [
    { label: "Tasks Completed", value: "18", icon: <Target className="w-5 h-5" /> },
    { label: "Hours Worked", value: "6.5", icon: <Clock className="w-5 h-5" /> },
    { label: "Productivity", value: "85%", icon: <TrendingUp className="w-5 h-5" /> },
    { label: "Team Members", value: "12", icon: <Users className="w-5 h-5" /> }
  ]


  return (
    <div className="min-h-full z-0 bg-gradient-to-br from-gray-50 via-purple-50/30 to-indigo-50/20 relative overflow-hidden">
      {/* Background Pattern */}
      ...
      <div className="absolute bottom-20 right-10 w-40 h-40 bg-[#c4b5fd]/10 rounded-full blur-xl animate-pulse" style={{ animationDelay: '1s' }} />
      <div className="absolute top-1/2 left-1/4 w-24 h-24 bg-[#1d0c37]/5 rounded-full blur-lg animate-pulse" style={{ animationDelay: '2s' }} />
      ...
      <div className="relative z-10 space-y-8">
        ...
        <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-8">
          <div className="text-center mb-2">
            <h1 className="text-4xl font-bold bg-gradient-to-r from-[#1d0c37] to-[#a98af7] bg-clip-text text-transparent mb-4">
              Welcome to your Dashboard
            </h1>
            <p className="text-gray-600 text-lg">
              Your productivity command center awaits
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8">
            {stats.map((stat, index) => (
              <div key={index} className="bg-white/50 rounded-xl p-4 text-center border border-[#a98af7]/10">
                <div className="flex items-center justify-center mb-2 text-[#a98af7]">
                  {stat.icon}
                </div>
                <div className="text-2xl font-bold text-[#1d0c37]">{stat.value}</div>
